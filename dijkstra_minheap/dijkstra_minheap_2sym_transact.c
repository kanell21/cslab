...
//<<<<<<<<<<<<<<<<================================= T R A N S A C T I O N A L     M E M O R Y         D E F I N I T I O N S =======================>>>>>>>>>>>>>>>>>>>>

    count_abort_t *counter;
    int count;
	counter = (count_abort_t *) malloc(sizeof(count_abort_t));
    counter->conflicts = 0; 
    counter->capacity = 0;  
    counter->explicits = 0;
    counter->debug = 0;
            ...
            for(int i=0;i<nb;i++)
            {
                                               ...
                                               graph->array[u].head[i].weight+ nodes[u].dist < nodes[v].dist)
                {
                    ...
                    // update distance value in min heap also
             	       ...
             	       gettimeofday(&ts,NULL);	
    				#endif
                    begin_transaction(2, &writelock, counter);
                    decreaseKey(minHeap, v, nodes[v].dist);
                    end_transaction(&writelock,counter);
                    #ifdef DECREASE
     	                gettimeofday(&tf,NULL);
            	        time=(tf.tv_sec-ts.tv_sec)+(tf.tv_usec-ts.tv_usec)*0.000001;
