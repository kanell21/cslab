...
//In order to check neighbours' heterogenia
    ...
    while (!isEmpty(minHeap))
    {
        ...
        __builtin_prefetch ((struct AdjListNode*)(graph->array[minHeap->array[0]->v].head), 1, 1);
   struct AdjListNode* pCrawl = graph->array[u].head;
        ...
        while (pCrawl != NULL)
        {
            ...
            if (isInMinHeap(minHeap, v) && nodes[u].dist != INT_MAX && 
                                          pCrawl->weight + nodes[u].dist < nodes[v].dist)
            {
                nodes[v].dist = nodes[u].dist + pCrawl->weight;
                nodes[v].prev=u;
